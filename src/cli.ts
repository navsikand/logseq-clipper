...
function printUsage(): void {
	const programName = DELIVERY_BACKEND === 'logseq' ? 'logseq-clipper' : 'obsidian-clipper';
	const appName = DELIVERY_BACKEND === 'logseq' ? 'Logseq' : 'Obsidian';
	const usage = `
Usage: ${programName} <url> [options]
...
Options:
  ...
  -o, --output <path>          Output .md file path (default: stdout)
      ...
      --vault <name>           ${appName} vault name
      --open                   Send to ${appName} instead of writing file
      --uri                    Use URI scheme instead of ${appName} CLI
      --silent                 Suppress ${appName} focus (URI mode)
