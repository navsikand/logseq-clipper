 ...
 * Obsidian backend.
 *
 * URI scheme:  obsidian://new?file=<path+name>  |  obsidian://daily?
 * Frontmatter: YAML via shared.ts::generateFrontmatter.
 * Delivery:    clipboard-first (&clipboard URI flag) with URI fallback.
 */
export const obsidianBackend: DeliveryBackend = {
	kind: 'obsidian',
	supportsClipboardDelivery: true,
	...
	async save(params: SaveParams): Promise<void> {
		const { fileContent, noteName, path, vault, behavior, silentOpen } = params;
		...
		if (isDailyNote) {
			...
			obsidianUrl = `obsidian://new?file=${encodeURIComponent(p + formattedNoteName)}`;
		}
